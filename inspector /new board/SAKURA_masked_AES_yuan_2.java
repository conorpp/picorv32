...
public class SAKURA_masked_AES_yuan_2 extends BasicSequence {
    ...
    public void run() {
        ...
        //Verdict setting: board should reply 8 bytes response; if we send the fixed message, check that response is ok
            ...
            if (HexUtils.startsWith(Arrays.copyOfRange(response, 1, 17), expectedCipherText)) { // fault injection verdict
            //if (!HexUtils.startsWith(Arrays.copyOfRange(test_fault, 1, 17), hex(DEFAULT_SAKURA_AES_KEY))) { // Yuan: instruction skip verdict
            //if (!HexUtils.startsWith(Arrays.copyOfRange(test_fault, 1, 17), hex(DEFAULT_SAKURA_AES_KEY))) { // Yuan: instruction skip verdict
